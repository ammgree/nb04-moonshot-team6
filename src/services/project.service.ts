...
export const getUserProjects = async ({
  ...
  const pageNum = Math.max(Number(page) ?? DEFAULT_PAGE, DEFAULT_PAGE);
  const pageSize = Math.min(
    Math.max(Number(limit) ?? MIN_PAGESIZE, MIN_PAGESIZE),
    MAX_PAGESIZE
  );
  if (!["created_at", "name"].includes(orderBy))
    throw new BadRequestError("잘못된 정렬 기준입니다");

  if (!["asc", "desc"].includes(order))
    throw new BadRequestError("잘못된 정렬 방향입니다");

  const offset = (pageNum - 1) * pageSize;

  const projects = await repo.getUserProjectsRepo({
    userId,
    orderBy,
    order,
    limit: pageSize,
